...
export default async function Cartes() {
  ...
  // Les puces « orphelines » n'ont de sens que lorsque le registre des
  // commerces est en service : tant qu'aucun commerce n'est déclaré, ne pas
  // être rattaché est l'état normal de toutes les puces, pas une alarme.
  const orphelines = registre.proprietesConnues
    ? registre.cartes.filter((c) => !c.commerce)
    : [];
        ...
        <>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <Mesure
              ...
              valeur={registre.proprietesConnues
                ? compteOuPhrase(registre.sansCommerce) : "—"}
              note={registre.proprietesConnues ? undefined : t.sansCommerceDetail}
              ton={registre.proprietesConnues && registre.sansCommerce > 0
                ? "negatif" : "neutre"}
            />
          </div>
