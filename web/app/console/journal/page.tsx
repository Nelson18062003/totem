...
import {
  CadreConsole, Cellule, EnTete, Etiquette, Panneau,
  RienADire, TableauQuiDefile,
} from "../gabarit";

export const dynamic = "force-dynamic";

type Recherche = { boitier?: string; jour?: string };
...
export default async function JournalDeLaConsole({
  ...
  const t = textesRegistre[langue].journal;

  const journal = await chargerJournal(
    {
      terminal: params.boitier ?? null,
      jour: params.jour ?? null,
    },
    langue,
  );

  // L'adresse porte la question : changer un filtre garde l'autre.
  const lien = (change: Recherche) => {
    const q = new URLSearchParams();
    const boitier = "boitier" in change ? change.boitier : params.boitier;
    const jour = "jour" in change ? change.jour : params.jour;
    if (boitier) q.set("boitier", boitier);
        ...
        <>
          ...
          >
            <div className="divide-hair">
              <Choix
                titre={t.terminalTitre}
                actif={journal.filtre.terminal ?? ""}
                options={[
                  { cle: "", libelle: t.terminalTous, href: lien({ boitier: undefined }) },
                  ...journal.terminaux.map((b) => ({
                    cle: b.id,
                    libelle: b.nom,
                    href: lien({ boitier: b.id }),
                  })),
                ]}
              />
              <Choix
