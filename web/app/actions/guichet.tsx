...
/**
 * Le guichet. Chaque geste ouvre la vraie session USSD dans la console, avec
 * le code du catalogue relevé sur le terrain (codes.py) déjà composé — les
 * menus qui s'affichent ensuite sont ceux de l'opérateur, pas les nôtres.
 * Rien n'est simulé, rien n'est deviné.
 */
export function Guichet({ carte }: { carte: Pick<Sim, "libelle" | "operateur"> }) {
  const op = carte.operateur;

  const vers = (code: string) => `/ussd?code=${encodeURIComponent(code)}`;

  const operations = [
    { titre: "Dépôt", sous: "Créditer un compte Mobile Money", Icone: IconArrowDown, code: codeUssd(op, "depot") },
    { titre: "Retrait", sous: "Chez un agent", Icone: IconWallet, code: codeUssd(op, "retrait") },
    { titre: "Transfert", sous: "Envoyer vers un numéro", Icone: IconArrowUp, code: codeUssd(op, "transfert") },
  ].filter((o) => o.code);
  ...
  return (
    ...
    <div className="flex flex-col gap-7 lg:grid lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start lg:gap-x-10">
              ...
              <li key={titre}>
                <Link href={vers(code)}
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-small text-ink-faint">{sous} · <span className="tabnums">{code}</span></p>
                  </div>
      ...
      <p className="text-caption leading-relaxed text-ink-faint lg:col-start-1">
        ...
        pose ses questions, vous répondez ; le code secret se compose sur son
        pavé au moment où l’opérateur le demande, et n’est enregistré nulle
        part.
      </p>
    </div>
  );
